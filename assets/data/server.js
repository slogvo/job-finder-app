  ...
  routes() {
    this.get('/api/product', (schema, req) => {
      return listProducts
        ...
        .filter(
          (item) =>
            removeVietnameseTones(item.companyName.toLowerCase()).includes(
              removeVietnameseTones(req.queryParams.s.toLowerCase()) || ''
            ) ||
            removeVietnameseTones(item.companyDescription.toLowerCase()).includes(
              removeVietnameseTones(req.queryParams.s.toLowerCase()) || ''
            ) ||
            removeVietnameseTones(item.companyLocation.toLowerCase()).includes(
              removeVietnameseTones(req.queryParams.s.toLowerCase()) || ''
            )
        );
    });
